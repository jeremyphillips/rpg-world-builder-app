...
| `Button`, `buttonVariants`, `ButtonProps`       | component  | Variants include `text` (standalone text actions with optional `tone`: `accent` \| `neutral` \| `danger`); `tone` only type-checks on `variant="text"`; 4 sizes             |
| `Link`, `textActionVariants`                    | component  | Design-system `<a>` — inline accent by default; `context="standalone"` for hover-underline text actions. Router links: `standaloneTextActionClasses()` from `@rpg/ui`       |
...
| `PreviewRail`                                   | component  | Structural content-preview card (header, identity, facts, grouped accordion, status panel, full-width action). No authoring semantics — dashboard owns Ready/Off copy.      |
...
| `NavSection`                                    | component  | Static sidebar section grouping (eyebrow label + stacked children)                                                                                                          |
| `SidebarNavSectionDisclosure`                   | component  | Controlled collapsible sidebar section (trigger + caret + content stack)                                                                                                    |
| `sidebarNavItemVariants`                        | util       | CVA for sidebar nav link presentation — inactive `text-sidebar-nav-item-fg` (`foreground-subtle` light / `muted-foreground` dark)                                           |
