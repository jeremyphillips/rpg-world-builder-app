...
const OBSOLETE_FOOTER_PATTERNS = [
...
function collectSourceFiles(dir: string, files: string[] = []): string[] {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    if (/\.(tsx?|jsx?)$/.test(entry)) {
      if (entry === 'dialog-panel-footer-migration.test.ts') {
        continue
      }
      files.push(fullPath)
    }
...
describe('overlay footer migration gate', () => {
  it('does not reference deleted sheet footer chrome symbols in product code', () => {
    const violations: string[] = []

    for (const root of SCAN_ROOTS) {
      for (const file of collectSourceFiles(root)) {
        const content = readFileSync(file, 'utf8')
        for (const pattern of OBSOLETE_FOOTER_PATTERNS) {
