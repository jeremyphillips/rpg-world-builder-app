...
| **Derived surfaces** | `surface-lift`, `surface-subtle`, `surface-muted`, …                         | `color-mix` from anchors — lift plane rises canvas toward white (light) or panel (dark) |
| **Semantic aliases** | `field-placeholder`, `field-fg-disabled`, `field-bg` → `surface-lift`        | Preserve component intent; field plane separate from surface-relative chrome            |
