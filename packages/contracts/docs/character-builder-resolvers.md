...
| `deriveEquipmentRecommendations`          | `resolvers/equipment/derive-equipment-recommendations.ts`            | Tiered picker recommendations (essential/strong/compatible/neutral/notRecommended) with reasons; inference-first plus authored `characterCreation.equipmentRecommendations` rules.                                                                                                                                 |
...
| `resolveEquipmentPickerItems`             | `resolvers/equipment/resolve-equipment-picker-items.ts`              | Annotates equipment rows with picker state + `searchText`; excludes kinds outside `EQUIPMENT_PICKER_SUPPORTED_KINDS`. Browse order: tier → reason → kind → weapon category (dual proficient only) → name (`compareEquipmentPickerItemsByRecommendation`); search scoring in the dashboard picker stays text-first. |
