...
- **Choose** — pick one or more values from available options: `Choose a rarity.` /
  `Choose at least one tool.`
- **Enter** — typed text/number values: `Enter a valid email address.`
- **Add** / **Remove** — repeatable array containers and collection actions:
  `Add at least one wealth tier.`
- **Select** — reserve for UI-selection mechanics (table rows, bulk selection,
  card pickers) — not default choice-field validation or placeholders.
- Multi-select count copy shares `choiceCountPhrase` in `messages.ts` for
  validation messages and form-layer constraint hints.
...
| At-least-one / add-one               | Tier-1 `minItems` and action-led "Add …" copy fit repeatable array containers; multi-select uses `minSelections` / `choiceCountPhrase` ("Choose at least …") |
...
| `invalid_type` (missing)                | multi                   | `Choose at least one {item label}.`                                |
...
| `too_small` array                       | multi                   | `Choose at least one {item label}.` (label singularized)           |
| `too_small` array min > 1               | multi                   | `Choose at least {min} {items label}.`                             |
| `too_big` array                         | multi                   | `Choose up to {max} {items label}.`                                |
| `too_small` / `too_big` array `exact`   | multi                   | `Choose {n} {items label}.`                                        |
| `too_small` array                       | array container         | `Add at least one {item label}.` (label singularized)              |
| `invalid_value` / `invalid_union` empty | choice                  | `Choose {a label}.`                                                |
| `invalid_value` / `invalid_union` empty | multi                   | `Choose at least one {item label}.`                                |
...
| `too_small` / `too_big` array `exact`   | array container         | `Add exactly {n} {items label}.`                                   |
