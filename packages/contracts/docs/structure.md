...
| Campaign-scoped character relationship edges (envelope, kind registry, draft, projection) | `rpg/runtime/character-relationships/` | `relationship.ts`, `definitions.ts`, `draft.ts`, `projection.ts`, `commands.ts` — vocabulary in `rpg/vocab/character-relationship/` |
| Character narrative generator contracts (tokens, fragments, facts, composition plan)      | `character-narrative/`                 | `vocabulary.ts`, `relationship-facts.ts`, `schemas.ts` — import via `@rpg/contracts/character-narrative`                            |
