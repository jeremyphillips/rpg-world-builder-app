...
describe('ruleset routes', () => {
  ...
  it('returns resolved character creation rules for an unpatched ruleset', async () => {
    ...
    expect(res.body.patch.characterCreation.proficiencyGrants.languages).toEqual({
      items: ['common'],
      categories: [],
    })
  ...
  it('returns active language vocabulary for a supported ruleset', async () => {
    const { agent } = await registerAndLoginTestUser(getApp())

    const res = await agent.get('/api/rulesets/srd-cc-5.2.1/languages').expect(200)

    expect(Array.isArray(res.body.languages)).toBe(true)
    expect(res.body.languages.some((language: { id: string }) => language.id === 'common')).toBe(
      true,
    )
    expect(
      res.body.languages.every(
        (language: { category?: string }) =>
          language.category === 'standard' || language.category === 'rare',
      ),
    ).toBe(true)
  })
  ...
  it('returns 404 for unsupported ruleset ids', async () => {
    ...
    await agent.get('/api/rulesets/unsupported-ruleset/languages').expect(404)
  })
