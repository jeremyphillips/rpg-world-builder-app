...
const organizationConnectedRegionsQuerySchema = z.object({
  page: z.coerce.number().int().min(1).default(1),
  pageSize: z.coerce.number().int().min(1).max(100).default(4),
})

export async function listOrganizationConnectedRegions(req: Request, res: Response): Promise<void> {
  const { campaignId, organizationId } = req.params as {
    campaignId: string
    organizationId: string
  }

  const queryResult = organizationConnectedRegionsQuerySchema.safeParse(req.query)
  ...
  const connectedRegions = await resolveOrganizationConnectedRegions({
    campaignId,
    organizationId,
  ...
  res.status(200).json(connectedRegions)
}
