...
import { listSubclasses, resolveSubclassesForCampaign } from './list-subclasses'
...
export async function getSubclassCampaignAccessAvailabilityHandler(
  req: Request,
  res: Response,
): Promise<void> {
  const { campaignId, classId, subclassId } = subclassRouteParams(req)
  ...
  const availability = await getContentCampaignAccessAvailability(
    subclassWriteConfig,
    campaignId,
    subclassId,
  )
