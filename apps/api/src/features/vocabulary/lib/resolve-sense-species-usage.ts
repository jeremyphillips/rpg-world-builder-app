...
export async function resolveSenseSpeciesUsageBatch(
  ...
  return getVocabularyUsageRegistration(SENSE_SET_ID).batchResolver(ctx, entryIds)
}
...
export async function resolveSenseSpeciesUsage(
  ...
  return getVocabularyUsageRegistration(SENSE_SET_ID).entryResolver(ctx, entryId)
}
