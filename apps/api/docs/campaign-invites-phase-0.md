...
> Phase 0 items marked **IMPLEMENTED** below shipped in phases 1–9 and follow-up
> work. Historical **DEFERRED** rows are retained for context.
...
| 7   | `ClientSession` propagation          | **IMPLEMENTED** | `MONGO_TRANSACTION_MODE` + propagation matrix for invite completion orchestrations                   |
...
Introduce `ClientSession` threading when deployment topology gains replica-set
transactions (`MONGO_TRANSACTION_MODE=auto` or `required`). See
[campaign-invites.md](./campaign-invites.md) onboarding completion and
`docs/environment.md` for configuration. Initial invite-repo scope is
