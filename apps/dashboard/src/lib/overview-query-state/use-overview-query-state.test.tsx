...
function renderOverviewQueryState(
  initialEntry = '/',
  options?: { allowedSortIds?: readonly string[]; mode?: 'url' | 'local' },
) {
...
describe('useOverviewQueryState', () => {
  ...
  describe('local mode', () => {
    it('keeps filter state in memory without writing the URL', async () => {
      const { result } = renderOverviewQueryState('/', { mode: 'local' })
    ...
    it('resets filters without navigation', () => {
      const { result } = renderOverviewQueryState('/', { mode: 'local' })

      act(() => {
        result.current.state.actions.setFilterValue('status', 'draft')
        result.current.state.actions.resetFilters()
      })

      expect(result.current.state.query.filters.status).toBeUndefined()
      expect(result.current.searchParams.get('status')).toBeNull()
    })
  })
})
