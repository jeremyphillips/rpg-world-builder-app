...
export const MEDIA_MANAGER_TOAST_IDS = {
  imageRemovedPrefix: 'media-image-removed',
...
export function resolveMediaRemovedToastId(): string {
  return `${MEDIA_MANAGER_TOAST_IDS.imageRemovedPrefix}-${crypto.randomUUID()}`
}
