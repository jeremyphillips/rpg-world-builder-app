export const equipmentInventorySummaryClasses = 'space-y-6'

export const equipmentInventorySummaryGroupClasses = 'space-y-2'

export const equipmentInventorySummaryListClasses = 'space-y-2'

export const equipmentInventorySummaryRowClasses =
  'flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border bg-card px-4 py-3'

export const equipmentInventorySummaryRowMetaClasses = 'flex flex-wrap items-center gap-2'

export const equipmentInventorySummarySourceClasses = 'text-muted-foreground'
