...
/** @deprecated Use `buildEquipmentInventoryViewModel`. */
export function buildEquipmentInventoryLayout(
  ...
  if (viewModel.startingEquipment.kind === 'gold_option') {
    return {
      mode: 'gold' as const,
      magicItems: viewModel.addedEquipment
        .filter((group) => group.group === 'magicItems')
        .map((group) => ({
          group: group.group,
          groupLabel: group.groupLabel,
          displays: group.entries.flatMap((entry) =>
            groupEquipmentInventoryRowsForDisplay(entry.rows, { allowCombinedRows: true }),
          ),
        })),
      purchased: viewModel.addedEquipment
        .filter((group) => group.group !== 'magicItems')
        .map((group) => ({
          group: group.group,
          groupLabel: group.groupLabel,
  ...
  const magicItems = viewModel.addedEquipment
    .filter((group) => group.group === 'magicItems')
    .map((group) => ({
      group: group.group,
      groupLabel: group.groupLabel,
  ...
  const purchased = viewModel.addedEquipment
    .filter((group) => group.group !== 'magicItems')
    .map((group) => ({
      group: group.group,
      groupLabel: group.groupLabel,
  ...
  return {
    ...
    startingPackage: viewModel.startingEquipment.group,
    magicItems,
    purchased,
  }
}
