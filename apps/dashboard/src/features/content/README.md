...
| [`MasterDetailListPanel`](./components/master-detail/master-detail-list-panel.tsx)                            | Bordered collection rail: fixed chrome outside a flex-capped shell; item list scrolls internally without a scrollbar gutter. One resolved `--master-detail-shell-max-block-size` fallback drives min-height floors and baseline `max-height`; in-container tighten uses scroll-column `cqh` without invalidating the fallback. Sticky `top` offsets with `--rpg-form-sticky-tabs-block-size`. `FormStickyScrollBody` clip prevents painting into the docked footer. Definite-host `--master-detail-list-max-block-size` overrides take priority. Whole-row selection, structured meta, empty copy. |
| [`MasterDetailEditorPanel`](./components/master-detail/master-detail-editor-panel.tsx)                        | Bordered detail rail when a row is selected; transparent centered empty state (Sparkles icon + vocab-driven heading/subhead) when nothing is selected. Optional line-3 broad campaign availability + **Change** and selected-row issue count via `MasterDetailEditorStatusRow`, overflow delete, selected row `FormItems`.                                                                                                                                                                                                                                                                         |
| [`MasterDetailAvailabilityHeaderLine`](./components/master-detail/master-detail-availability-header-line.tsx) | Presentation-only broad `Available` / `Unavailable` row + **Change** affordance for nested editors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| [`MasterDetailEditorStatusRow`](./components/master-detail/master-detail-editor-status-row.tsx)               | Detail identity status line: broad campaign availability + **Change** and selected-row issue count.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| [`MasterDetailGrid`](./components/master-detail/master-detail-grid.tsx)                                       | Shared grid wrapper (`items-start`, `md:grid-cols-3` → **1/3 list rail + 2/3 detail**). Defines `--master-detail-list-max-block-size` (default `28rem`); hosts may override with a definite length. Sticky-chrome forms publish `--rpg-content-top-inset` / `--rpg-content-bottom-inset` via `@rpg/ui/form` docked-footer shell composition.                                                                                                                                                                                                                                                       |
| [`MasterDetailEditorShell`](./components/master-detail/master-detail-editor-shell.tsx)                        | Shared bordered detail rail (`self-start`, content-sized): identity header (line-3 availability/issue status + overflow delete), and body slot or empty state.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
`ariaLabel` (nav accessible name), and `itemNoun` (`MasterDetailItemNounTerm` via
`defineMasterDetailItemNoun` — drives list/detail empty copy and delete labels).
Pass `ContentFormCtx.embeddedSeedRowIds`
