...
| [`MasterDetailListPanel`](./components/master-detail/master-detail-list-panel.tsx)                            | Bordered collection rail: fixed chrome outside a bounded item-list scroll body; viewport fallback caps subtract optional `--rpg-content-bottom-inset` (conservative, not remaining height to footer). Definite-host `--master-detail-list-max-block-size` overrides take priority. Whole-row selection, structured meta, empty copy.     |
...
| [`MasterDetailGrid`](./components/master-detail/master-detail-grid.tsx)                                       | Shared grid wrapper (`items-start`, `md:grid-cols-3` → **1/3 list rail + 2/3 detail**). Defines `--master-detail-list-max-block-size` (default `28rem`); hosts may override with a definite length. Sticky-chrome forms publish `--rpg-content-bottom-inset` via `@rpg/ui/form` docked-footer shell composition.                         |
