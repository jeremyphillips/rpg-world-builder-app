...
| [`MasterDetailListPanel`](./components/master-detail/master-detail-list-panel.tsx)                            | Bordered collection rail: fixed chrome outside a flex-capped shell; item list scrolls internally without a scrollbar gutter. One resolved `--master-detail-shell-max-block-size` drives `max-height` and min-height floors; sticky `top` offsets with `--rpg-form-sticky-tabs-block-size`. Inside docked-footer form scroll bodies, caps track the scroll column via container queries. Definite-host `--master-detail-list-max-block-size` overrides take priority. Whole-row selection, structured meta, empty copy. |
