...
| [`MasterDetailEditorPanel`](./components/master-detail/master-detail-editor-panel.tsx)                | Bordered detail rail when a row is selected; transparent centered empty state (Sparkles icon + vocab-driven heading/subhead) when nothing is selected. Overflow delete, validation banner, selected row `FormItems`.                                                                                                                     |
