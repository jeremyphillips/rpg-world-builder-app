...
| [`MasterDetailListPanel`](./components/master-detail/master-detail-list-panel.tsx)                            | Bordered collection rail: fixed chrome (`listTitle` + Add, optional availability counts) outside a bounded item-list scroll body; whole-row selection with tint/inset accent, structured meta subtitle (`eyebrow · sourceLabel`), and `No {noun plural} added.` when empty.                                                              |
...
| [`MasterDetailGrid`](./components/master-detail/master-detail-grid.tsx)                                       | Shared grid wrapper (`items-start`, `md:grid-cols-3` → **1/3 list rail + 2/3 detail**). Defines `--master-detail-list-max-block-size` (default `28rem`); hosts may override with a definite length (`32rem`, `60dvh`, or a host-owned CSS variable).                                                                                     |
