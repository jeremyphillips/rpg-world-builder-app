...
export type ContentOverviewRowSelectionState = Record<string, boolean>

export type UseContentOverviewSelectionOptions = {
...
export function useContentOverviewSelection<T extends { id: string }>({
  visibleRowIds,
  selectionLimit,
}: UseContentOverviewSelectionOptions) {
  const [selectionMode, setSelectionMode] = useState(false)
  const [rowSelection, setRowSelection] = useState<ContentOverviewRowSelectionState>({})
  const [selectedRows, setSelectedRows] = useState<T[]>([])

  useEffect(() => {
    setRowSelection((current: ContentOverviewRowSelectionState) => {
  ...
  const handleRowSelectionStateChange = useCallback((state: ContentOverviewRowSelectionState) => {
  ...
  const selectionCapDescriptionId = useMemo(
    () => `content-overview-selection-cap-${selectionLimit}`,
    [selectionLimit],
  )
