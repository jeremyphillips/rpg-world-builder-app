...
The **Traits** tab is a master-detail editor over the species's embedded `traits` array, built on the shared content master-detail abstraction (see [`content` README](../README.md#master-detail-abstraction)): a selectable list on the left, the selected trait's form on the right. New traits are authored (`kind: custom` in hidden form state). Existing SRD grant traits (for example Darkvision) keep a derived editor that explains generated display and offers an override, without a kind selector or Custom/Grant list eyebrow. It binds to the parent form via `useFieldArray`, so global save and validation are unchanged from the previous inline array. Campaign availability is body `available?` only (dialog via `access={{ kind: 'availability', fieldName: 'available' }}`); traits inherit species player visibility at runtime.

The **Heritage** tab edits an optional singular `heritage` object: scalar **name** (with a hint for lineage/ancestry wording) and **description** at the top, then a master-detail list over `heritage.options` (authored custom trait rows; the form contract does not allow grant options). An empty state offers **Set up heritage**; once present, options use **Add option**. No list eyebrows on options. Each heritage option carries body-embedded `campaignAccess` (full player-access dialog via the body-bound adapter — not overlay PATCH). The authoring rail filters on **local** option access; species overlay narrowing surfaces as inherited restriction copy and in runtime effective access.
