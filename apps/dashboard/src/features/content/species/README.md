...
The **Heritage** tab edits an optional singular `heritage` object: scalar **name**, **kind** (lineage/ancestry), and **description** at the top, then a master-detail list over `heritage.options` (trait rows, same field helpers as the Traits tab). An empty state offers **Add lineage/ancestry**; once present, options use **Add lineage** or **Add ancestry** depending on kind. No list eyebrows — kind is on the scalar form.
