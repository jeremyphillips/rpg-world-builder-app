...
| Grant row helpers         | `content/lib/forms/grants/grant-form-schema.ts` (`GRANT_TYPES` for consumer pickers, `GRANT_ROW_TYPES` / `GRANT_ROW_TYPE_LABELS` when equipment rows are representable, `GrantType`, `GRANT_TYPE_LABELS`), `grant-form-fields.ts`, `grant-form-values.ts`, `equipment-grant-form-fields.ts`, `equipment-grant-form-values.ts`, `equipment-grant-form-labels.ts` |
