...
[`ContentFormHost`](../src/features/content/lib/forms/shells/host/content-form-host.tsx)
...
### Field copy (placeholders, hints, validation)

Routine choice-field copy is generated by `@rpg/ui/form` from field semantics —
do not hand-author generic `Choose…` / `Select…` placeholders or constraint hints
when the field config already declares `min` / `max` / `required`.

- **Placeholders** — omit on `select` / `combobox`; the renderer defaults to
  `Choose {noun}…`. Use explicit `placeholder` only for domain examples or search
  affordances (`Search …`).
- **Constraint hints** — omit when `min` / `max` on multi-select `chips` /
  `combobox` fully describe the rule; add `noun: { singular, plural }` when the
  visible `label` is not grammatical alone (`Armor` → `piece of armor`).
- **Validation** — keep Zod schemas message-free; tier-1 copy comes from
  `fieldValidationMessages` via the field error map. Reuse
  `fieldValidationMessages` / `grantFieldRequiredSelectMessage` in `.refine`
  hooks instead of literal strings.
- **Drift guard** — `field-copy-drift-guard.test.ts` fails on generic hand-authored
  copy in `*-form-fields.ts`, `*-form-labels.ts`, and `*-form-constants.ts`.
...
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Classes (hub)            | `classes/lib/class-form-fields.ts` (schema + tabs); tab modules `class-basics-form-fields.ts`, `class-proficiencies-form-fields.ts`, `class-spellcasting-form-fields.ts`           | aligned |
...
| Grant row helpers         | `content/lib/forms/grants/` — union/core at root (`grant-form-schema.ts`, `grant-form-fields.ts`, `grant-template-registry.ts`, …); typed splits in `grants/equipment/` and `grants/proficiency/` (includes `tool-proficiency-pool-form-*`, `proficiency-equipment-link-cue.tsx`) |
...
Optional. When present, create/edit shells switch to `WidePage` and an `xl` persistent PreviewRail (sheet + compact **Preview** trigger below that). Register `buildIdentity` and `buildSections` keyed so `buildSections` keys **exactly equal** `buildTabs` ids (`null` = explicit opt-out). Optional `PreviewResources` resolves async/domain data for `buildSections` (class: `ClassPreviewResourcesProvider` in `class-preview-resources.tsx`). Optional `renderPlayerPreview` supplies the player-facing modal — omit until the type has a player surface. Shared copy comes from `CONTENT_TYPE_TERMS`. Publish readiness uses `resolveContentPublishSchema` (the same helper as the Publish action). Class is the reference implementation (`class-preview-projection.ts`); types without `preview` keep NarrowPage.
...
| Module             | Path                                                                                                                                         |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Requirement editor | `feats/lib/requirement-editor-form-schema.ts`, `-values.ts`, `-form.ts` (+ `-constants.ts`); UI in `feats/components/requirement-editor.tsx` |
