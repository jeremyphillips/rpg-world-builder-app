...
| Campaign access          | `content/lib/campaign-access/campaign-access-form-fields.ts`, `campaign-access-form-visibility.ts`, `campaign-access-labels.ts` — disclosure via `disclosure: { variant: 'summary' }` on the group (see [containers.md](../../../../packages/ui/docs/forms/containers.md#group-disclosure)) | aligned |
