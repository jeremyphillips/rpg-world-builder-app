...
```text
pnpm lint-staged → regenerate JSON schemas (when @rpg/contracts Zod inputs change) → pnpm typecheck:affected → pnpm test:affected → pnpm gate:fallow-dupes → pnpm gate:fallow-health
```
