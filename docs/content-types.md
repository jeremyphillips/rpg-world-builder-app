...
- `overviewUsageScope` is **metadata only** — it does not change which sources run. V1 character-only batch uses `characters` so overview chrome can say “My characters” honestly.
